...
Extra puzzle sets for stressing the solvers, in the same format as
`puzzles/{easy,medium,hard}.txt`: one puzzle per line, 81 characters,
...
## Running one solver over these puzzles

```
python -m solvers.cli ac3 benchmarks/hard.txt
python -m solvers.cli backtracking benchmarks/hard.txt --timeout 30
python -m solvers.cli min_conflicts benchmarks/hard.txt --repeats 3 --seed 0
```
